...
## 2026-04-09 - 远程 Jupyter 执行采用半自动化模式

- 状态：已接受
- 背景：当前 AMD 集群通过 HTTP 暴露 Jupyter Lab，实例创建需要手动完成。API 可以访问，但基于当前 VS Code 工具面无法完全自动化远程 shell 执行。
- 决策：远程验证采用混合工作流：手动创建实例、在有凭据时通过脚本访问 Jupyter API，并为终端执行生成命令包。
- 影响：后续每次远程测试都应记录哪些步骤是自动化完成的，哪些步骤是在 Jupyter 内手动执行的。
